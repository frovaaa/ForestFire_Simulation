...
fill_grid <- function(forest_grid, pos_func, prob_func, cell_type) {
  max_y <- dim(forest_grid$status)[1]
  max_x <- dim(forest_grid$status)[2]

  for (y in 1:max_y) {
    for (x in 1:max_x) {
      if (pos_func(y, x, max_y)) {
        forest_grid$status[y, x] <- cell_type
...
initial_state <- fill_grid(initial_state, sin_pos, function(y, x) 0.1, 1)
...
simulate <- function() {
  ...
  for (i in 1:N) {
    ...
    saveGIF(ani.fun(), movie.name = "forest_simulation.gif", interval = 1)
  }
}
